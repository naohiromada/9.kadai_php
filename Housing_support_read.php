...
$sql = 'SELECT * FROM housing_support_table 
 LEFT OUTER JOIN (SELECT housing_support_id, COUNT(id) AS cnt 
 FROM  housing_support_like_table GROUP BY  housing_support_id) AS likes 
 ON housing_support_table.id =  likes.housing_support_id';
...
if ($status == false) {
    ...
    foreach ($result as $record) {
        ...
        // いいねボタン
        $output .= "<td>
              <a href='Housing_support_like_create.php?id={$record["id"]}'>いいね{$record["cnt"]}</a></td>"; // cntカラムの数値（いいね数）を追加

        // $output .= "<td>
        //               <a href='Housing_support_like_create.php?user_id={$user_id}&Housing_support_id={$record["id"]}'>
        //  いいね{$record["cnt"]} </a></td>"; // cntカラムの数値（いいね数）を追加
        // edit deleteリンクを追加
        $output .= "<td>
              <a href='Housing_support_edit.php?id={$record["id"]}'>編集</a>
              </td>";
        $output .= "<td>
              <a href='Housing_support_delete.php?id={$record["id"]}'>削除</a>
              </td>";
        // $output .= "</tr>";
        $output .= "</tr>";
    }
